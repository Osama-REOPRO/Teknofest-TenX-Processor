module post_norm( clk, fpu_op, opas, sign, rmode, fract_in, exp_in, exp_ovf,opa_dn, opb_dn, rem_00, div_opa_ldz, output_zero, out,
...
output		f2i_out_sign;
   ...
   endcase     
    ...
    assign exp_in_80	= exp_in[7] & !(|exp_in[6:0]); // 10000000
    assign exp_out_ff       = &exp_out;
    assign exp_out_00       = !(|exp_out);
    assign exp_out_fe       = &exp_out[7:1] & !exp_out[0]; // 11111110
    assign exp_out_final_ff = &exp_out_final; // defined later
     
    assign fract_out_7fffff = &fract_out; //11111111111111111111111 (fract out is 23 bits so first zero (7) is ignored)
    assign fract_out_00     = !(|fract_out);
    assign fract_in_00      = !(|fract_in);

    assign rmode_00 = (rmode==2'b00); // round_nearest_even
    assign rmode_01 = (rmode==2'b01); // round_to_zero
    assign rmode_10 = (rmode==2'b10); // round_up
    assign rmode_11 = (rmode==2'b11); // round_down
     
    // Fasu Output will be denormalized ...
    assign dn = !op_mul & !op_div & (exp_in_00 | (exp_next_mi[8] & !fract_in[47]) );
    // assign dn = not mul and not div and (exp is zero or exp
    ...
    assign overflow  = op_div ? overflow_fdiv : 
                       (ovf0 | ovf1);
    ...
    assign ine =	op_f2i ? f2i_ine :
                    ...
                    (
                        ...
                        (s & !dn) | 
                        max_num | 
                        (op_div & !rem_00) // If operation is div and remainder is not zero
                    );
                    
     
// Debugging Stuff ( REMOVED; CHECK IN ORIGINAL)
endmodule