...
output  reg [7:0]	exp_dn_out; // de normalized exponent output
output	reg	sign;
output	reg	nan_sign, result_zero_sign; 
output	reg  fasu_op;    // Operation (add/sub) register
    ...
    // ---------------------------------------------------------------------
    // Normalize -> if exponent is all zero then the number is denormalized
    ...
    assign exp_diff1a = exp_diff1-1; // We minus one to account for the implicit hidden bit
    ...
    assign adj_op_tmp = { ~op_dn, adj_op, 3'b0 }; // recover hidden bit (op_dn) and 3 guard, round and sticky bits 
     
    // adj_op_out is 27 bits wide, so can only be shifted 27 bits to the right
    assign exp_lt_27	= exp_diff  > 8'd27; // 27 bits is the max ammount of shift
    ...
    // ---------------------------------------------------------------------
    // Sort operands (for sub only) Always make A bigger than B
    ...
    // Fix sign for ZERO result - to figure out the sign of 0: +-0
    always @(posedge clk) begin
        signa_r <= signa;
        signb_r <= signb;
        add_r <= add;
        result_zero_sign <=	( add_r &  signa_r &  signb_r) | //adding two (-) numbers
                    (!add_r &  signa_r & !signb_r) | // subtracting (-) a and (+) b
                    ( add_r & (signa_r |  signb_r) & (rmode==3)) | // adding atleast one (-) while rounding down
                    (!add_r & (signa_r == signb_r) & (rmode==3)); // subtracting same sign while rounding down
